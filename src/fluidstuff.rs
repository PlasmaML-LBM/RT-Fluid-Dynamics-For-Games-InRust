...
impl fluidobj {
    pub fn full_step(&mut self, dt: f64) {
        //vel_step(&mut self.u, &mut self.v, &mut self.p_u, &mut self.p_v, self.visc, dt);
        dens_step(&mut self.den, &mut self.p_den, &mut self.u, &mut self.v, self.diff, dt);
    }
    pub fn new() -> fluidobj {
        let mut x = fluidobj { p_den: [0_f64; Nex as usize], den: [0_f64; Nex as usize], u: [0_f64; Nex as usize], v: [0_f64; Nex as usize], p_u: [0_f64; Nex as usize], p_v: [0_f64; Nex as usize], diff: 0.3_f64, visc: 0.3_f64 };
...
fn diffuse(b: i32, den: &mut [f64; Nex as usize], p_den: &mut [f64; Nex as usize], diff: f64, dt: f64) {
    let a: f64 = dt * diff * ((N*N) as f64);
    for k in 0..20 {
        for i in 1..=N {
            for j in 1..=N {
                den[IX(i, j)] = 
                (p_den[IX(i, j)] + a*(den[IX(i-1, j)] + den[IX(i+1, j)] + den[IX(i, j-1)] + den[IX(i, j+1)])) / (1 as f64 + 4 as f64 * a);
            }
        }
        set_bnd(b, den);
    }
...
// /*
fn advect(b: i32, den: &mut [f64; Nex as usize], p_den: &mut [f64; Nex as usize], u_vel: &mut [f64; Nex as usize], v_vel: &mut [f64; Nex as usize], dt: f64) {
    ...
    for i in 1..=N {
        for j in 1..=N {
            ...
            let x = if (x < 0.5) {0.5} else if (x > N as f64 + 0.5) {N as f64 + 0.5} else {x};
            let y = if (x < 0.5) {0.5} else if (x > N as f64 + 0.5) {N as f64 + 0.5} else {x};
            ...
            den[IX(i, j)] = s0 * (t0 * p_den[IX(i0, j0)] + t1 * p_den[IX(i0, j1)]) + s1 * (t0 * p_den[IX(i1, j0)] + t1 * p_den[IX(i1, j1)]);
        }
    }
    set_bnd(b, den);
}
...
fn project(u: &mut [f64; Nex as usize], v: &mut [f64; Nex as usize], p: &mut [f64; Nex as usize], div: &mut [f64; Nex as usize]) {
    ...
    for k in 0..20 {
        for i in 1..=N {
            for j in 1..=N {
                p[IX(i, j)] = (div[IX(i, j)] + p[IX(i-1, j)] + p[IX(i+1, j)] + p[IX(i, j-1)] + p[IX(i, j+1)]) / 4_f64
            }
        }
        set_bnd(0, p);
    }
...
pub fn dens_step(den: &mut [f64; Nex as usize], p_den: &mut [f64; Nex as usize], u_vel: &mut [f64; Nex as usize], v_vel: &mut [f64; Nex as usize], diff: f64, dt: f64) {
...
pub fn vel_step(u_vel: &mut [f64; Nex as usize], v_vel: &mut [f64; Nex as usize], p_u_vel: &mut [f64; Nex as usize], p_v_vel: &mut [f64; Nex as usize], visc: f64, dt: f64 ) {
    ...
    std::mem::swap(v_vel, p_v_vel);
    std::mem::swap(u_vel, p_u_vel);
    advect(1, u_vel, p_u_vel, &mut p_u_vel.clone(), p_v_vel, dt);
    advect(1, v_vel, p_v_vel, p_u_vel, &mut p_v_vel.clone(), dt);
    project(u_vel, v_vel, p_u_vel, p_v_vel);
}

fn set_bnd(b: i32, arr: &mut [f64; Nex as usize]) {
    for i in 1..=N {
        arr[IX(0, i)] = if (b == 1) {-arr[IX(1, i)]} else {arr[IX(1, i)]};
        arr[IX(N+1, i)] = if (b == 1) {-arr[IX(N, i)]} else {arr[IX(N, i)]};
        arr[IX(i, 0)] = if (b == 2) {-arr[IX(i, 1)]} else {arr[IX(i, 1)]};
        arr[IX(i, N+1)] = if (b == 2) {-arr[IX(i, N)]} else {arr[IX(i, N)]};    
    }
